...
(function (window, $) {
    ...
    var ngMoveSelectionHandler = function ($scope, elm, evt, grid) {
        ...
        if (document.activeElement.tagName === "INPUT" || document.activeElement.tagName === "SELECT" || document.activeElement.tagName === "A" || document.activeElement.tagName === "TEXTAREA") {
            return true;
        }
        ...
        if (offset) {
            ...
            if (r.beforeSelectionChange(r, evt)) {
                ...
                else {  // AS: #22531: https://pushkarkinikar.wordpress.com/2014/07/03/ng-grid-scrolling-issue//
    ...
    angular.module('ngGrid.services').factory('$domUtilityService', ['$utilityService', '$window', function ($utils, $window) {
        ...
        domUtilityService.AssignGridContainers = function ($scope, rootEl, grid) {
            ...
            /*var scopeDereg = $scope.$watch(function () {
                var result = grid._viewPortScrollLeft || 0;

                setTimeout(function () {
                    if (grid &&
                        grid.$viewport) {
                        grid._viewPortScrollLeft = grid.$viewport.scrollLeft();
                    }
                }, 250);
    ...
    var ngAggregate = function (aggEntity, rowFactory, rowHeight, groupInitState) {
        this.rowIndex = 0;
        this.offsetTop = (rowFactory.showRowDataHeader ? rowFactory.rowDataHeaderHeight : 0) + (this.rowIndex * rowHeight);
    ...
    var ngColumn = function (config, $scope, grid, domUtilityService, $templateCache, $utils) {
        ...
        self.headerColor = config.headerColor;
        ...
        self.gripOnMouseDown = function (event) {
            ...
            // C.C. Remove drag drop on grip mouse down
            // Bug 18139 - Reports Data Grid: sometimes changing the column widths does not behave as expected
            if (!grid.config.jqueryUIDraggable) {
                var columns = grid.$root.find('.ngHeaderSortColumn'); //have to iterate if using removeEventListener
                angular.forEach(columns, function (col) {
                    if (col.className && col.className.indexOf("ngHeaderSortColumn") !== -1) {
                        ...
                        if (col.removeEventListener) { //IE8 doesn't have drag drop or event listeners
                            col.removeEventListener('dragstart', grid.eventProvider.dragStart);
                        }
                    }
                });
                if (navigator.userAgent.indexOf("MSIE") !== -1) {
                    //call native IE dragDrop() to start dragging
                    grid.$root.find('.ngHeaderSortColumn').unbind('selectstart');
                }
            }
    ...
    var ngGrid = function ($scope, options, sortService, domUtilityService, $filter, $templateCache, $utils, $timeout, $parse, $http, $q) {
        var defaults = {
            ...
            */
            ...
            //Row height of aggregate row headers in grid.
            ...
            // the height of the row data header
            rowDataHeaderHeight: 50,

            // the row data header template
            rowDataHeaderTemplate: undefined,

            // True to show the row data header, false otherwise
            showRowDataHeader: false,

            // Prevent auto sorting on group
            preventAutoSortOnGroup: false,

            // Suppress rebuild grid on column update
            suppressRebuildOnColumnUpdate: false,

            // Suppress emiting alternating row styles
            suppressAlternatingRowStyles: false     
        },
            self = this;
        self.maxCanvasHt = 0;
        //self vars
        ...
        self.calcMaxCanvasHeight = function () {
            ...
            if (self.config.groups.length > 0) {
                calculatedHeight = self.rowFactory.parsedData.filter(function (e) {
                    return !e[NG_HIDDEN] && !e.isAggRow;
                }).length * self.config.rowHeight;
                calculatedHeight = calculatedHeight + self.rowFactory.parsedData.filter(function (e) {
                    return !e[NG_HIDDEN] && e.isAggRow && !(e.parent && e.parent.collapsed);
                }).length * self.config.aggregateRowHeight;
            } else {
                calculatedHeight = self.filteredRows.length * self.config.rowHeight;
            }
            calculatedHeight = calculatedHeight + (self.config.showRowDataHeader ? self.config.rowDataHeaderHeight : 0);
        ...
        self.configureColumnWidths = function () {
            ...
            // C.C Take grouping columns into account
            // Without this a horizontal scroll bar appears
            // when columns are grouped
            if (self.config.groups &&
                self.config.groups.length) {
                totalWidth += (self.config.groups.length * 25) + 1;
            }
        ...
        self.init = function () {
            return self.initTemplates().then(function () {
                ...
                $scope.$watch('columns', function (a) {
                    if (!$scope.isColumnResizing && !$scope.suppressRebuildOnColumnUpdate) {
        ...
        $scope.aggregateRowHeight = self.config.aggregateRowHeight;
        ...
        $scope.suppressRebuildOnColumnUpdate = self.config.suppressRebuildOnColumnUpdate;
        ...
        // Row data header
        $scope.rowDataHeaderHeight = self.config.rowDataHeaderHeight;
        $scope.showRowDataHeader = self.config.showRowDataHeader;
        $scope.preventAutoSortOnGroup = self.config.preventAutoSortOnGroup;
        $scope.suppressAlternatingRowStyles = self.config.suppressAlternatingRowStyles;
        ...
        $scope.adjustScrollTop = function (scrollTop, force) {
            ...
            // C.C. grid is raising two scroll events when data is paged due to scrolling to the bottom.
            // Only raise the event when force is false.
            if (!force && scrollTop > 0 && self.$viewport[0].scrollHeight - scrollTop <= (self.$viewport.outerHeight() + 1)) {
                $scope.$emit('ngGridEventScroll');
            }

            var rowIndex = 0;

            // C.C calculate the correct row index when we have groups whose height is different from the row height.
            if ($scope.configGroups &&
                $scope.configGroups.length > 0 &&
                self.config.rowHeight != self.config.aggregateRowHeight &&
                self.rowFactory.parsedData) {
                var rowOffset = 0,
                    index = -1,
                    parsedData = self.rowFactory.parsedData;
                ...
                for (var idx = 0; idx < parsedData.length; idx++) {
                    ...
                    if (e.isAggRow) {
                        ...
                        height = self.config.aggregateRowHeight;
                    } else {
                        height = self.config.rowHeight;
                    }

                    if (height && !e[NG_HIDDEN]) {
                        index = index + 1;
        ...
        $scope.showReportHeader = function () {
            return self.config.showReportHeader;
        };

        $scope.topPanelHeight = function () {
            return self.config.showReportHeader ? (self.config.showGroupPanel === true ? self.config.headerRowHeight + 32 : self.config.headerRowHeight) : 0;
        };
        ...
        $scope.groupBy = function (col) {
            ...
            if (!$scope.preventAutoSortOnGroup) {
    ...
    var ngRow = function (entity, config, selectionProvider, rowIndex, $utils) {
        ...
        this.offsetTop = (config.showRowDataHeader ? config.rowDataHeaderHeight : 0) + (this.rowIndex * config.rowHeight);
    ...
    ngRow.prototype.alternatingRowClass = function () {
        ...
        var classes = {
            ...
            'even': !this.config.suppressAlternatingRowStyles && isEven,
            'odd': !this.config.suppressAlternatingRowStyles && !isEven,
    ...
    var ngRowFactory = function (grid, $scope, domUtilityService, $templateCache, $utils) {
        ...
        self.aggregateRowHeight = grid.config.aggregateRowHeight;
        self.rowDataHeaderHeight = grid.config.rowDataHeaderHeight;
        self.showRowDataHeader = grid.config.showRowDataHeader;        
        self.rowConfig = {
            ...
            aggregateRowHeight: grid.config.aggregateRowHeight,
            rowDataHeaderHeight: grid.config.rowDataHeaderHeight,
            showRowDataHeader: grid.config.showRowDataHeader,
            suppressAlternatingRowStyles: grid.config.suppressAlternatingRowStyles
        };
        ...
        self.buildAggregateRow = function (aggEntity, rowIndex) {
            ...
            if (!agg) {
                // build the row
                agg = new ngAggregate(aggEntity, self, self.rowConfig.aggregateRowHeight, grid.config.groupsCollapsedByDefault);
                self.aggCache[aggEntity.aggIndex] = agg;
            }
            agg.rowIndex = rowIndex;
            agg.offsetTop = (self.showRowDataHeader ? self.rowDataHeaderHeight : 0) + (rowIndex * self.rowConfig.aggregateRowHeight);
            return agg;
        };
        ...
        self.renderedChange = function () {
            ...
            var countRow = 0;
            var tempRow;
            for (var i = 0; i < self.renderedRange.topRow; i++) {
                tempRow = temp[i];
                if (tempRow) {
                    if (tempRow.isAggRow) {
                        countAggRow++;
            ...
            var offsetTop = grid.config.showRowDataHeader ? grid.config.rowDataHeaderHeight : 0;
            offsetTop = offsetTop + (countAggRow * grid.config.aggregateRowHeight) + (countRow * grid.config.rowHeight);
            for (var i = self.renderedRange.topRow; i < self.renderedRange.bottomRow; i++) {
                tempRow = temp[i];
                if (tempRow) {
                    if (tempRow.isAggRow) {
                        tempRow.offsetTop = offsetTop;
                        offsetTop = offsetTop + grid.config.aggregateRowHeight;
                    } else {
                        tempRow.offsetTop = offsetTop;
                        offsetTop = offsetTop + grid.config.rowHeight;
                    }
                    rowArr.push(tempRow);
                }
            }
        ...
        self.renderedChangeNoGroups = function () {
            var rowArr = [];
            var rowDataHeaderHeight = grid.config.showRowDataHeader ? grid.config.rowDataHeaderHeight : 0;
            for (var i = self.renderedRange.topRow; i < self.renderedRange.bottomRow; i++) {
                if (grid.filteredRows[i]) {
                    grid.filteredRows[i].rowIndex = i;
                    grid.filteredRows[i].offsetTop = rowDataHeaderHeight + (i * grid.config.rowHeight);
                ...
                for (var prop in g) {
                    ...
                    if (prop === NG_FIELD || prop === NG_DEPTH || prop === NG_COLUMN) {
                        ...
                        var agg = self.buildAggregateRow({
                            ...
                            aggLabelFilter: g[NG_COLUMN] ? g[NG_COLUMN].aggLabelFilter : null
                        }, 0);
                        ...
                        if (agg.parent) {
                            // Reduce groups expanding/collapsing on load
                ...
                for (var z = 0; z < groups.length; z++) {
                    if (!cols[z].isAggCol && z <= maxDepth) {
                        cols.splice(0, 0, new ngColumn({
                            ...
                            headerColor: grid.config.headerColor
    ...
    var ngStyleProvider = function ($scope, grid) {
        ...
        $scope.rowStyle = function (row) {
            var ret = { "top": row.offsetTop + "px", "height": row.isAggRow ? $scope.aggregateRowHeight : $scope.rowHeight + "px" };
            if (row.isAggRow) {
                ...
                return {
                    pre: function ($scope, iElement) {
                        ...
                        if ($scope.col.enableCellEdit) {
                            ...
                            html = html.replace(EDITABLE_CELL_TEMPLATE, $scope.col.editableCellTemplate.replace(COL_FIELD, $scope.col.field ? 'row.entity.' + $scope.col.field : ''));
                        } else {
                            html = cellTemplate;
                        }
    ...
    ngGridDirectives.directive('ngHeaderCell', ['$compile', function ($compile) {
        var ngHeaderCell = {
            ...
            compile: function () {
                return {
                    pre: function ($scope, iElement) {
                        var linkFuncCache = $scope.linkFuncCache;
                        var linkFunc;                        
                        
                        if (!linkFuncCache.headerCellLinkFuncs) {
                            linkFuncCache.headerCellLinkFuncs = {};                            
                        }

                        var tmplKey = new String($scope.col.headerCellTemplate);
                        linkFunc = linkFuncCache.headerCellLinkFuncs[tmplKey];

                        if (!linkFunc) {
                            linkFunc = $compile($scope.col.headerCellTemplate);
                            linkFuncCache.headerCellLinkFuncs[tmplKey] = linkFunc;
                        }                        

                        linkFunc($scope, function (clone) {
                ...
                return {
                    pre: function ($scope, iElement) {
                        ...
                        if ($scope.row.isAggRow) {
                            ...
                            if (!CUSTOM_FILTERS.test(html)) {
                                if (!linkFuncCache.aggRowTemplateLinkFunc) {
                                    linkFuncCache.aggRowTemplateLinkFunc = $compile(html);
                                }

                                linkFuncCache.aggRowTemplateLinkFunc($scope, function (clone) {
                                    iElement.append(clone);
                                });
                            } else {
                            ...
                            if (!linkFuncCache.rowTemplateLinkFunc) {
                                linkFuncCache.rowTemplateLinkFunc = $compile($templateCache.get($scope.gridId + 'rowTemplate.html'));
                            }                            
                            
                            linkFuncCache.rowTemplateLinkFunc($scope, function (clone) {                                
                                iElement.append(clone);
                            });                            
                        }
        ...
        return function ($scope, elm) {
            ...
            function scroll(evt) {
                ...
                if ($scope.forceSyncScrolling) {
                    ...
                    scrollTimer = setTimeout(ensureDigest, 250);
                }
    ...
    angular.module('ngGrid').run(['$templateCache', function ($templateCache) {
        ...
        $templateCache.put('gridTemplate.html',
          "<div class=\"ngTopPanel\" ng-class=\"{'ui-widget-header':jqueryUITheme, 'ui-corner-top': jqueryUITheme}\" ng-style=\"topPanelStyle()\" ng-show=\"showReportHeader()\">\r" +
          "\n" +
          "    <div class=\"ngGroupPanel\" ng-if=\"showGroupPanel()\" ng-style=\"groupPanelStyle()\">\r" +
          "\n" +
          "        <div class=\"ngGroupPanelDescription\" ng-if=\"configGroups.length == 0\">{{i18n.ngGroupPanelDescription}}</div>\r" +
          "\n" +
          "        <ul ng-if=\"configGroups.length > 0\" class=\"ngGroupList\">\r" +
          "\n" +
          ...
          "\n" +
          "        <div ng-if=\"showRowDataHeader\" ng-include=\"gridId + 'rowDataHeaderTemplate.html'\"></div>\r" +
          "\n" +
          "        <div ng-style=\"rowStyle(row)\" ng-repeat=\"row in renderedRows\" ng-click=\"row.toggleSelected($event)\" rowindex=\"{{row.rowIndex}}\" ng-class=\"row.alternatingRowClass()\" ng-row></div>\r" +
          "\n" +
          "    </div>\r" +
          "\n" +
          "</div>\r" +
        ...
        $templateCache.put('headerRowTemplate.html',
          "<div ng-style=\"{ height: col.headerRowHeight, background: col.headerColor }\" ng-repeat=\"col in renderedColumns\" ng-class=\"col.colIndex()\" class=\"ngHeaderCell\">\r" +
          "\n" +
          "\t<div class=\"ngVerticalBar\" ng-style=\"{height: col.headerRowHeight, background: col.headerColor}\" ng-class=\"{ ngVerticalBarVisible: !$last }\">&nbsp;</div>\r" +
          "\n" +
        ...
        $templateCache.put("rowDataHeaderTemplate.html",
            "<div ng-style=\"{ height: rowDataHeaderHeight }\" ng-repeat=\"col in renderedColumns\" ng-class=\"col.colIndex()\" class=\"ngCell {{col.cellClass}}\">" +
            "	<div ng-style=\"{ height: rowDataHeaderHeight }\"></div>" +
            "</div>"
          );
