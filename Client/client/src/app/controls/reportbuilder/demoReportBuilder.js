...
(function () {
    ...
    angular.module('app.demoReportBuilder', ['ui.router', 'spApps.reportServices', 'mod.common.ui.spTreeview', 'mod.common.ui.spFieldsManager', 'mod.common.ui.spRelationshipPicker', 'mod.common.ui.spEntityComboPicker', 'mod.common.spEntityService', 'mod.ui.spTreeviewManager', 'sp.navService', 'sp.spNavHelper'])
                                                      ...
                                                      $stateParams, spNavService)
     {
         ...
         $scope.reportPropertyMode = { reportName: '', reportDescription: '', rootEntityId: 0, disableOkButton: true, showAll: false, type: null };
         $scope.reportBuilderMode = { reportEntity: null, treeNode: null, selectTreeNode: null, selectTreeNodeId : 0, treeviewOptions: null, fileManagerOptions: null};
         ...
         $scope.pickRelationship = function() {
             if ($scope.selectedEntity) {
                 ...
                 var relationshipPickerOptions = { entityTypeId: $scope.selectedEntityId, selectedRelationshipIds: ids };
         ...
         $scope.setFileManagerOption = function(fields, selectedNodeId) {
             $scope.reportBuilderMode.fileManagerOptions = { "nodeFields": fields, "selectedNodeId": selectedNodeId, "reportEntity": $scope.reportBuilderMode.reportEntity };
         };
         ...
         $scope.$watch('reportBuilderMode.selectTreeNode', function() {
             ...
             if ($scope.reportBuilderMode.selectTreeNode) {
                 ...
                 else
                 {
                     ...
                     spEntityService.getEntity($scope.reportBuilderMode.selectTreeNode.etid, spResource.makeTypeRequest()).then
                    (
                        function (typeEntity) {
             ...
             resource.getLookups().forEach(
                  function (relationship) {
                      if (!relationship.isHidden()) {
                          ...
                          fields.push({ "id": spUtils.newGuid(), "fid": $scope.nameFieldId, "fname": name, "ftype": "InlineRelationship", "isreq": false, "group": group, "svid": 0, "relid": relationship.getEntity().eid().id(), "ftid": fromType.eid().id(), "ttid": toType.eid().id(), "aggm": "", "inrep": false, "inanal": false });
                      }
                  }
             );
             ...
             resource.getChoiceFields().forEach(
                function (relationship) {
                    if (!relationship.isHidden()) {
                        ...
                        var group = relationship.getFieldGroupEntity() ? relationship.getFieldGroupEntity().getName() : 'Relationship Fields'; //Relationship Fields
                        var name = relationship.getName() ? relationship.getName() : toType.name;
                        fields.push({ "id": spUtils.newGuid(), "fid": $scope.nameFieldId, "fname": name, "ftype": "ChoiceRelationship", "isreq": false, "group": group, "svid": 0, "relid": relationship.getEntity().eid().id(), "ftid": fromType.eid().id(), "ttid": toType.eid().id(), "aggm": "", "inrep": false, "inanal": false });
                    }
                }
            );
         ...
         $scope.getFieldJson = function (field, resource) {
             ...
             return { "id": spUtils.newGuid(), "fid": fid, "fname": fname, "ftype": ftype, "isreq": field.isRequired(), "group": group, "svid": 0, "relid": 0, "ftid": 0, "ttid": 0, "aggm": "", "inrep": false, "inanal": false, "dname": displayName };
         };
