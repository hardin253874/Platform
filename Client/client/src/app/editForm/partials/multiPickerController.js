...
angular.module('app.editForm.multiPickerController', ['mod.app.editForm', 'sp.navService'])
    .controller('multiPickerController',
        function ($scope, $state, spFieldValidator, spEditForm, spNavService) {
            'use strict';
            
            var form, navItem, isSourceRelationshipFilterControl;

            // init
            spEditForm.commonRelFormControlInit($scope, { validator: validate, areCreating: $state.current.name === 'createForm' });
            var customValidationMessages = $scope.customValidationMessages;

            navItem = spNavService.getCurrentItem();
            form = spUtils.result(navItem, 'data.formControl');
