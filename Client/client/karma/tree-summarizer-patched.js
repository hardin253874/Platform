...
TreeSummary.prototype = {
    ...
    convertToTree: function (summaryMap, arrayPrefix) {
        ...
        function getParent(key, seen, root) {
            var dirName = path.dirname(key);
            var parentPath = dirName + SEP;
            if (key === dirName || parentPath === SEP + SEP) {
                parentPath = root.name;
            }
            var parent = seen[parentPath];
            if (!parent) {
                parent = new Node(parentPath, 'dir');
                var grandparent = getParent(dirName, seen, root);
        ...
        Object.keys(summaryMap).forEach(function (key) {
            ...
            seen[key] = node;
            nodes.push(node);
            parent = getParent(key, seen, root);
        ...
        // start where the content gets interesting
