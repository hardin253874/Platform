			...
			try
			{
				...
				using ( IDbCommand command = databaseManager.CreateCommand( commandText ) )
				{
					using ( IDataReader reader = command.ExecuteReader( ) )
					{
						...
						while ( reader.Read( ) )
						{
							var transaction = new HistoricalTransaction( reader );
...
SELECT
	dbo.fnName( @userId )";
				...
				foreach ( string sol in users.OrderBy( k => k ) )
				{
					if ( !string.IsNullOrEmpty( sol ) )
					{
						UserFilters.Add( new FilterObject( sol, true, UserFilterUpdate ) );
					}
				}
				...
				foreach ( string sol in tenants.OrderBy( k => k ) )
				{
					if ( !string.IsNullOrEmpty( sol ) )
					{
						TenantFilters.Add( new FilterObject( sol, true, TenantFilterUpdate ) );
					}
				}
				...
				foreach ( string sol in spids.OrderBy( k => k ) )
				{
					if ( !string.IsNullOrEmpty( sol ) )
					{
						SpidFilters.Add( new FilterObject( sol, true, SpidFilterUpdate ) );
					}
				}
				...
				foreach ( string sol in programs.OrderBy( k => k ) )
				{
					if ( !string.IsNullOrEmpty( sol ) )
					{
						ProgramFilters.Add( new FilterObject( sol, true, ProgramFilterUpdate ) );
					}
				}
				...
				foreach ( string sol in logins.OrderBy( k => k ) )
				{
					if ( !string.IsNullOrEmpty( sol ) )
					{
						LoginFilters.Add( new FilterObject( sol, true, LoginFilterUpdate ) );
					}
				}
