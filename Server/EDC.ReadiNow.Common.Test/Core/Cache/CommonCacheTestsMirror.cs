...
namespace EDC.ReadiNow.Test.Core.Cache
{
    /// <summary>
    ...
    [RunAsDefaultTenant]
    [TestFixture]
    public class CommonCacheTestsMirror     // A mirror of CommonCacheTests for caches that are defined in EDC.ReadiNow.Common
    {
        ...
        {
            if (cacheName == "DelayedInvalidateCache")  // We don't care about removes for this cache
                Assert.Pass();
        ...
        [Test]
        [TestCaseSource( "Caches" )]
        [Ignore("These are not yet implemented for these caches")]
        ...
        public IEnumerable<TestCaseData> Caches( )
        {
            ...
            {
                new PerTenantCacheFactory { Inner = inner },
                new PerTenantNonSharingCacheFactory { Inner = inner },
                new DelayedInvalidateCacheFactory { Inner = inner },
            };
