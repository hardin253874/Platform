...
-- Get structure level names following the hierarchy relationship in the reverse direction
CREATE FUNCTION [dbo].[fnGetStructureLevelNamesRev]
(
	@TenantId BIGINT,
	@ResourceId BIGINT,
...
BEGIN
	...
	BEGIN
		...
		AS (
			...
			INNER JOIN Paths p ON svTree.ToId = p.Id
			JOIN Data_NVarChar svName ON svTree.FromId = svName.EntityId AND svName.TenantId = @TenantId AND svName.FieldId = @NameFieldId
			WHERE svTree.TypeId = @StructureHierarchyRelationshipId
				AND svTree.TenantId = @TenantId				
				AND CHARINDEX( ',' + CAST ( svTree.FromId AS NVARCHAR( MAX ) ) + ',', p.[PathIds] ) = 0
		)
