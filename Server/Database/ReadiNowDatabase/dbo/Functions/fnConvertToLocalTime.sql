...
CREATE FUNCTION [dbo].[fnConvertToLocalTime] (
       @utcDate DATETIME,
...
BEGIN
       ...
       SELECT TOP 1 @return = dbo.fnFromUnixTime(dbo.fnUnixTimeStamp(@utcDate) + tz.Gmt_offset )
       FROM TimeZone tz 
		JOIN Zone z ON tz.Zone_id = z.Zone_id
		JOIN TimeZoneMap tzm ON z.Zone_name = tzm.Olson
       WHERE tz.Time_start < dbo.fnUnixTimeStamp(@utcDate) 
