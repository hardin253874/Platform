...
CREATE PROCEDURE [dbo].[spRevertTo]
...
BEGIN
	...
	BEGIN
		IF ( @tenantId IS NULL )
		BEGIN
			SET @context = 'Reverting to transaction ' + CAST( @transactionId AS VARCHAR( 20 ) )
		END
		ELSE
		BEGIN
			IF ( @tenantId = -1 )
			BEGIN
				SET @context = 'Reverting to transaction ' + CAST( @transactionId AS VARCHAR( 20 ) ) + ' for multiple tenants'
			END
			ELSE
			BEGIN
				SET @context = 'Reverting to transaction ' + CAST( @transactionId AS VARCHAR( 20 ) ) + ' for tenant ' +  CAST( @tenantId AS VARCHAR( 20 ) )
			END
		END
	END
	...
	EXEC spCreateRestorePoint @tenantId, DEFAULT, DEFAULT, DEFAULT, @transactionId, @tranId OUTPUT
